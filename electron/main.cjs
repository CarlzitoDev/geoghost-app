...
ipcMain.handle("location:get", async () => {
  // Try with tunnel (iOS 17+)
  if (rsdHost && rsdPort) {
    try {
      const output = await run(`pymobiledevice3 developer dvt simulate-location get --rsd ${rsdHost} ${rsdPort}`, 10000);
      const parsed = parseLocationOutput(output);
      if (parsed) return { ok: true, ...parsed };
    } catch (err) {
      console.log("RSD get-location failed:", err.message);
    }
  }

  // Try starting tunnel first
  try {
    const tunnel = await startTunnel();
    const output = await run(`pymobiledevice3 developer dvt simulate-location get --rsd ${tunnel.host} ${tunnel.port}`, 10000);
    const parsed = parseLocationOutput(output);
    if (parsed) return { ok: true, ...parsed };
  } catch (err) {
    console.log("Tunnel get-location failed:", err.message);
  }

  // Legacy fallback
  try {
    const output = await run("pymobiledevice3 developer dvt simulate-location get", 10000);
    const parsed = parseLocationOutput(output);
    if (parsed) return { ok: true, ...parsed };
  } catch {
    try {
      const output = await run("pymobiledevice3 developer simulate-location get", 10000);
      const parsed = parseLocationOutput(output);
      if (parsed) return { ok: true, ...parsed };
    } catch (err) {
      return { ok: false, error: err.message };
  ...
  return { ok: false, error: "Could not read device location" };
});
...
function parseLocationOutput(output) {
  ...
  // Try text patterns like "latitude: 37.123  longitude: -122.456"
  const latMatch = output.match(/latitude[:\s]+(-?[\d.]+)/i);
  const lngMatch = output.match(/longitude[:\s]+(-?[\d.]+)/i);
